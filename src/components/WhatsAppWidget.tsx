...
export default function WhatsAppWidget() {
  const whatsappUrl = `https://wa.me/${storeInfo.whatsapp}?text=${encodeURIComponent(
    "Hello Sharda Medical Store, I'd like to order medicines / inquire about services."
  )}`;
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 group flex flex-col items-end">
      ...
      <div className="mb-2 bg-slate-900 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-md opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-200 pointer-events-none whitespace-nowrap border border-slate-800">
        💬 Order via WhatsApp
      </div>
